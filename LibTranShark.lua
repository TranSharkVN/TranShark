...
local txtbox = tab:Textbox("Textbox","Nhập Gì Đó?",function(val)
	print(val)
end)
lib:Nof("notifi",3)
]]
...
function hoho:DraggingEnabled(frame, parent)
	...
	frame.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputTyput.UserInputState == Enum.UserInpe.MouseMovement then
			dragInput = input
		end
	end)
...
function hoho:Window(hub, the_game_name, main_color)
	...
	GameName.Text = hub or "Tran Shark"
	...
	FastLoadText.Text = "  Tải Nhanh (PC):"
